...
class ViajesController extends AppController {
	...
	public $paginate = array(
		'conditions' => array(
			'Viajes.total_distance >' => 0,
			'Viajes.total_time >' => 0,
			'Viajes.average_speed >' => 0
		),
		'order' => array('Viajes.start_date' => 'DESC'),
		'limit' => 15
	);
    ...
    {
	    parent::initialize();
	    $this->Viajes = TableRegistry::get('Api.Viajes');
	}
	public function index()
	{
		$viajes = $this->paginate('Api.Viajes');	
		$this->set('viajes', $viajes);
	}
	public function view($viaje_id = null)
	{
		if(!$viaje = $this->Viajes->findById($viaje_id)->first())
			$this->redirect('/viajes');
		$this->set('viaje', $viaje);
		$this->set('title_for_layout', 'Ruta '.$viaje->ruta);
	}
