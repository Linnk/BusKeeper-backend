<footer class="bg-gray">
<?php /*
	<div class="container">
		<div class="row text-xs-center">
			<div class="col-sm-4">
				<a class="btn btn-link text-white" href="/faq">Preguntas frecuentes</a>
			</div>
			<div class="col-sm-4">
				<a class="btn btn-link text-white"  href="/terms">Términos y condiciones</a>
			</div>
			<div class="col-sm-4">
				<a class="btn btn-link text-white"  href="/privacy">Aviso de privacidad</a>
			</div>
		</div>
	</div>
*/ ?>
	<div class="text-xs-center">
		<div class="container pt-1 pb-2">
			<p>© BusKeeper - <small>Creador por: <a href="http://twitter.com/Linnk" target="_blank" title="Ignacio Benavides">@Linnk</a>, <a href="http://twitter.com/Bogue89" target="_blank" title="Jorge Benavides">@Bogue89</a> y <a href="http://twitter.com/CarlosImarch" target="_blank" title="Carlos Imarch">@CarlosImarch</a></small></p>
			<a href="/api">API Pública (beta)</a> &nbsp; <a rel="license" href="http://creativecommons.org/licenses/by-nc-sa/3.0/deed.en_US" title="BusKeeper data is licensed under a Creative Commons Attribution-NonCommercial-ShareAlike 3.0 Unported License."><img alt="Creative Commons License" style="border-width:0" src="http://i.creativecommons.org/l/by-nc-sa/3.0/80x15.png"></a>
		</div>
	</div>
</footer>